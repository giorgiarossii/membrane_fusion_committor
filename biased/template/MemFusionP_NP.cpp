...
namespace PLMD
{
    namespace colvar
    {
        ...
        {
            std::vector<AtomNumber> NP, TAILS;
            std::vector<double> NSMEM, DSMEM, HMEM, RCYLMEM, ZETAMEM, ONEOVERS2C2CUTOFF, ZSHIFT;
        ...
        PLUMED_REGISTER_ACTION(memFusionP, "MEMFUSIONP_NP")

        void memFusionP::registerKeywords(Keywords &keys)
        {
            Colvar::registerKeywords(keys);
            keys.add("atoms", "NP", "all the NP beads of the system.");
            ...
            keys.add("optional", "ZETAMEM", "fraction to which a slice is filled upon the addition of the first polar atom into the slice");
            ...
            keys.add("optional", "ZSHIFT", "shift in the z direction for the cylinder (default is 0.0).");
        }
        ...
        memFusionP::memFusionP(const ActionOptions &ao) : PLUMED_COLVAR_INIT(ao)
        {
            ...
            parseAtomList("NP", NP);
            if (NP.empty())
                error("NP has no atoms specified.");    
            ...
            parseVector("ZSHIFT", ZSHIFT);
            if (ZSHIFT.size() > 1)
                error("ZSHIFT cannot take more than one value.");
            if (ZSHIFT.size() == 0)
                ZSHIFT.push_back(0.0);
        ...
        {
            // Print a start-of-calculate message
            /*************************
             *                        *
             *         System         *
             *                        *
            ...
            // Box dimensions.
            double Lx = getBox()[0][0], Ly = getBox()[1][1];

            double sumZ = 0.0;

            for (unsigned i = 0; i < NP.size(); i++) {
                sumZ += getPosition(i)[2];
            }
            double ZNP = sumZ / NP.size();
            // Use ZNP as the membrane center:
            double ZMems = ZNP + ZSHIFT[0];


           // SE VOGLIO CHE SEGUA ANCHE SU XY
            ...
            double XNP = sumX / NP.size();
            double YNP = sumY / NP.size();

            //Use these as the center-of-mass for NP:
            double Xcyl_NP = XNP;
            double Ycyl_NP = YNP;
            ...
            unsigned membraneBeads = NP.size();
            ...
            //double firstSliceZ_Mem = ZMems+ DSMEM[0] / 2.0;
            // double firstSliceZ_Mem = ZMems;
            ...
            for (unsigned i = 0; i < TAILS.size(); i++)
            {
                ...
                PositionS_Mem = (ZTailDistance) / DSMEM[0];
               ...
               
                {
                    ...
                    for (unsigned s = s1_Mem[i]; s <= s2_Mem[i]; s++)
                    {
                        ...
                        // using this x the slices where symmetrically placed around Zmems

                        x = (ZTailDistance - (s * DSMEM[0])) / DSMEM[0];
...
// NEW CODE HERE AS WELL
                ...
                double Xcyl_Mem = Xcyl_NP;
                double Ycyl_Mem = Ycyl_NP;
            ...
            // Center of the cylinder. XY components are calculated (or defined).
